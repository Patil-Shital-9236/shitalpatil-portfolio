...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  useLayoutEffect(() => {
    const cards = scrollerRef.current
      ? Array.from(scrollerRef.current.querySelectorAll('.scroll-stack-card') as NodeListOf<HTMLElement>)
      : [];
    ...
    const updateMeasurements = () => {
      ...
      const currentScrollY = useWindowScroll ? window.scrollY : (scrollerRef.current?.scrollTop || 0);

      // Temporarily remove transforms to get accurate rect during HMR/re-renders/resize
      cards.forEach(card => {
      ...
      endElementTopRef.current = endElement
        ? endElement.getBoundingClientRect().top + currentScrollY
        : 0;
    ...
    // Call one more time after a short delay to catch late-loading fonts/images
